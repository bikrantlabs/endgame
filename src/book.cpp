...
// PolyGlot Zobrist random numbers (781 entries, public domain)
// Sub-arrays: RandomPiece[768], RandomCastle[4], RandomEnPassant[8], RandomTurn[1]
static const uint64_t PolyRand[781] = {
...
// PolyGlot piece encoding: bp=0, wp=1, bn=2, wn=3, bb=4, wb=5,
//                          br=6, wr=7, bq=8, wq=9, bk=10, wk=11
static int poly_piece(Color c, PieceType pt) { return pt * 2 + (1 - c); }

static uint64_t polyglot_hash(const Position &pos) {
  uint64_t key = 0;
  ...
  for (int c = 0; c < COLOR_NB; ++c) {
    for (int pt = PAWN; pt <= KING; ++pt) {
      ...
      while (bb) {
        int sq = unset_lsb(bb);
        int idx = poly_piece(static_cast<Color>(c), static_cast<PieceType>(pt)) * 64 + sq;
        key ^= PolyRand[idx];
      }
    }
  }
  ...
  // Castling
  if (pos.castling_rights & WK_CASTLE) key ^= PolyRand[768 + 0];
  if (pos.castling_rights & WQ_CASTLE) key ^= PolyRand[768 + 1];
  if (pos.castling_rights & BK_CASTLE) key ^= PolyRand[768 + 2];
  if (pos.castling_rights & BQ_CASTLE) key ^= PolyRand[768 + 3];

  // En passant — only if an enemy pawn can actually capture
  if (pos.ep_square != NO_SQUARE) {
    int ep_file = sq_file(pos.ep_square);
    int ep_rank = sq_rank(pos.ep_square);
    Color us = pos.side_to_move;
    Color them = static_cast<Color>(1 - us);

    // Check if there is an enemy pawn adjacent to the EP square
    int left_sq  = (ep_file > 0) ? pos.ep_square - 1 : NO_SQUARE;
    int right_sq = (ep_file < 7) ? pos.ep_square + 1 : NO_SQUARE;

    bool can_capture = false;
    if (left_sq != NO_SQUARE && sq_rank(left_sq) == ep_rank) {
      int rank_adj = (us == WHITE) ? ep_rank - 1 : ep_rank + 1;
      int adj_sq = sq_of(sq_file(left_sq), rank_adj);
      if (test_bit(pos.pieces[them][PAWN], adj_sq))
        can_capture = true;
    }
    if (!can_capture && right_sq != NO_SQUARE && sq_rank(right_sq) == ep_rank) {
      int rank_adj = (us == WHITE) ? ep_rank - 1 : ep_rank + 1;
      int adj_sq = sq_of(sq_file(right_sq), rank_adj);
      if (test_bit(pos.pieces[them][PAWN], adj_sq))
        can_capture = true;
    }

    if (can_capture)
      key ^= PolyRand[772 + ep_file];
  }
...
#ifdef EMBED_BOOK
extern const unsigned char book_data[];
extern const size_t book_data_size;
#endif

static FILE *book_file = nullptr;
static const unsigned char *book_ptr = nullptr;
static size_t book_raw_size = 0;
static int book_size = 0; // number of entries (16 bytes each)
...
static bool read_entry(int n, BookEntry &e) {
  const unsigned char *src = nullptr;

  if (book_ptr) {
    src = book_ptr + n * 16;
  } else if (book_file) {
    if (fseek(book_file, n * 16, SEEK_SET) != 0)
      return false;
    static thread_local unsigned char buf[16];
    if (fread(buf, 1, 16, book_file) != 16)
      return false;
    src = buf;
  } else {
    return false;
  ...
  e.move = ((uint16_t)src[8] << 8) | src[9];
  e.weight = ((uint16_t)src[10] << 8) | src[11];

  e.learn = 0;
  for (int i = 12; i < 16; ++i)
    e.learn = (e.learn << 8) | src[i];
...
// Find the first entry index with matching key, or -1
static int book_find_first(uint64_t key) {
  int lo = 0, hi = book_size - 1;
  while (lo < hi) {
    int mid = (lo + hi) / 2;
    BookEntry e;
    if (!read_entry(mid, e))
  ...
  if (lo < 0 || lo >= book_size)
    return -1;
  BookEntry e;
  if (!read_entry(lo, e))
    return -1;
  return (e.key == key) ? lo : -1;
}

// ── PolyGlot move → engine Move ───────────────────────────────────────

static Move poly_to_engine_move(uint16_t poly_move, const Position &pos) {
  int from = ((poly_move >> 6) & 0x3F); // bits 6-11
  int to   = (poly_move & 0x3F);        // bits 0-5
  int promo = (poly_move >> 12) & 0x7;  // bits 12-14

  if (from == to)
    return Move{}; // invalid/null

  // Castling: PolyGlot encodes as king-from, rook-to
  if (from == E1 && to == H1 && pos.piece_type_on(WHITE, E1) == KING)
    return make_move(E1, G1, KING_CASTLE);
  if (from == E1 && to == A1 && pos.piece_type_on(WHITE, E1) == KING)
    return make_move(E1, C1, QUEEN_CASTLE);
  if (from == E8 && to == H8 && pos.piece_type_on(BLACK, E8) == KING)
    return make_move(E8, G8, KING_CASTLE);
  if (from == E8 && to == A8 && pos.piece_type_on(BLACK, E8) == KING)
    return make_move(E8, C8, QUEEN_CASTLE);

  // Detect captures
  bool is_cap = pos.is_occupied(to) ||
                (pos.piece_type_on(pos.side_to_move, from) == PAWN &&
                 sq_file(from) != sq_file(to) && to == pos.ep_square);

  // Promotion
  if (promo != 0) {
    PieceType promo_pt = static_cast<PieceType>(promo); // 1=KNIGHT..4=QUEEN
    MoveFlag flag = is_cap ? static_cast<MoveFlag>(PROMO_CAP_N + promo - 1)
                           : static_cast<MoveFlag>(PROMO_N + promo - 1);
    return make_move(from, to, flag);
  }

  // EP capture
  if (pos.piece_type_on(pos.side_to_move, from) == PAWN &&
      sq_file(from) != sq_file(to) && to == pos.ep_square) {
    return make_move(from, to, EP_CAPTURE);
  }

  if (is_cap)
    return make_move(from, to, CAPTURE);

  // Double pawn push
  if (pos.piece_type_on(pos.side_to_move, from) == PAWN &&
      abs(sq_rank(to) - sq_rank(from)) == 2) {
    return make_move(from, to, DOUBLE_PUSH);
  }

  return make_move(from, to, QUIET);
}

// ── Public API ─────────────────────────────────────────────────────────

bool book_open(const std::string &path) {
  if (book_file || book_ptr)
    return true;

#ifdef EMBED_BOOK
  if (path.empty()) {
    book_ptr = book_data;
    book_raw_size = book_data_size;
    book_size = (int)(book_raw_size / 16);
    return true;
  }
#endif
  ...
  book_file = f;
  book_raw_size = (size_t)file_size;
  book_size = (int)(book_raw_size / 16);
...
bool book_opened() { return (book_file || book_ptr) && book_size > 0; }

Move book_probe(const Position &pos) {
  if (!book_opened())
    return Move{};

  uint64_t key = polyglot_hash(pos);
  int first = book_find_first(key);
  if (first < 0)
    return Move{};
  ...
  for (int i = first; i < book_size; ++i) {
    ...
    candidates.push_back({e.move, e.weight});
  }
  ...
  for (auto &c : candidates) {
    cumulative += c.weight;
    if (pick < cumulative) {
      Move m = poly_to_engine_move(c.poly_move, pos);
      if (m == Move{})
        return Move{};
      ...
      MoveList ml;
      gen_legal_moves(check_pos, ml);
      for (int i = 0; i < ml.count; ++i) {
        if (ml[i] == m)
          return m;
      }
      return Move{}; // book move is illegal, fall back to search
    }
  }
